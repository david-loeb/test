  ...
  text: | 
    <style> 
      ...
      @media (max-width: 575.98px) {
        .quarto-dashboard .bslib-grid .min-height-card {
          grid-template-rows: minmax(250px, max-content) !important;
        }
      }
    </style>
---
...
```{ojs}
//| output: false
// This and next cell query the containing card's height/width when window 
// resizes and set plot height/width accordingly to fill card. Resizing is a 
// bit choppy. Alternative is set fixed height and use Observable's dynamic
// width variable, then just define a viewBox on SVG to 0 0 width height (or maybe 
// instead just define the height and width attrs?) Much smoother resizing but 
// will not fill card perfectly on height unless at perfect window size
container = document.querySelector('#ojs-cell-5');
```
...
### States Plot {.min-height-card}

```{ojs}
{
  const margin = ({top: 20, right: 20, bottom: 30, left: 40});  // Dimensions
  const tickNumY = Math.max(height / 50, 2);
  const tickNumX = Math.max(width / 100, 2);
